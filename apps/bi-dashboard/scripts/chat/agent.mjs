...
const ResponseSchema = z.object({
  ...
  chartType: z
    ...
    .describe(
      'The chart format to render. If the user explicitly named one ("pie chart", "line chart"/"trend", "donut", "bar chart"), match it exactly -- never silently substitute a different one. Otherwise, when a chart is wanted but no format was named, CHOOSE the form that actually fits what this data is showing, the way an analyst would -- do not default to bar out of habit: a day-by-day/date-over-time series (a trend, "over the last N days") -> line; a small number of categories that together make up a whole (order status mix, a channel/region split with ~2-6 slices) -> donut; a ranked comparison across many categories or entities (revenue by product, top customers, order count by region) -> bar. "none" if intent is "answer" and no chart was requested.',
    ),
...
Beyond the standard dashboard bundle, query_semantic_layer also supports a flexible "metric by dimension" breakdown -- pass metric ("revenue" | "order_count" | "avg_order_value" | "units" | "customer_count") together with groupBy ("region" | "product" | "customer" | "status" | "day") for questions that don't fit the fixed bundle, e.g. "revenue by status", "order count per customer", "units sold by product", "daily revenue trend", "number of customers per region". groupBy:"customer" genuinely JOINS orders with users -- use it for any real per-customer breakdown instead of guessing. "customer_count" (the number of DISTINCT customers, not orders) is only valid with groupBy "region", "status", or "day" -- it does NOT exist for groupBy:"customer" (each row is already one customer) or groupBy:"product" (order_items has no customer link); if the user asks for a customer count broken down by product, say plainly that this isn't available rather than inventing or approximating a number for it. CRITICAL -- when a user asks for "number of customers" alongside another metric (e.g. "customers and revenue per region"), that means metric:"revenue" with extraMetrics:["customer_count"] (or vice versa) on ONE call, so both numbers come from the SAME real query and both render in the table -- never state a customer count in your narrative that didn't come back from a customer_count field in the tool result; if customer_count isn't supported for the requested groupBy, say so explicitly instead of guessing or estimating one. Use sortDirection ("most"/"least") and limit for ranking questions. Always use this instead of inventing numbers or approximating from the fixed bundle when the user's question names a metric/dimension combination the fixed bundle doesn't cover.
...
- When no specific format was named but a chart/report was requested, pick the chart type that actually fits the data being shown instead of defaulting to bar every time: a time-series/trend over days -> line; a handful of categories that add up to a whole (status mix, a small region/channel split) -> donut; a ranked comparison across many entities (top customers/products, a breakdown by category) -> bar. Choosing the right form for the data IS part of a correct answer here, not a cosmetic afterthought.
